jQuery().ready(function (){ 
	jQuery("#listt_gigi").jqGrid({ 
		url:'t_gigi_pasien/masterXml/t_gigi_oral_model', 
		...
		colModel:[ 
				...
				{name:'gambar',index:'gambar', width:50},
			...
			onSelectRow:function(){
				...
				img.src = './assets/images/gigi_pasien/oral/'+myCellData+"?"+timestamp;
				img.width = 400;
				img.height = 350;
	...
	$("#listt_gigi .icon-edit").live('click', function(p){
		...
		$('#dialogt_gigi').attr('Title','Edit Foto Oral Gigi');
	...
	$('#t_gigiadd').click(function(){
		...
		// $("#main_form_gigi_pasien").load('t_gigi_pasien/add/1/Oral'+'?_='+(new Date()).getTime()+'&kd_pasien='+pasien+'&kd_puskesmas='+puskesmas);
		// $('#main_form_gigi_pasien').show();
		
		$('#dialogt_gigi').attr('Title','Tambah Foto Oral Gigi');

		$('#del_dialogt_gigi').hide();
		$('#ins_dialogt_gigi').show();
		$("#ins_dialogt_gigi").load('t_gigi_pasien/add/1/Oral'+'?_='+(new Date()).getTime()+'&kd_pasien='+pasien+'&kd_puskesmas='+puskesmas);
		$("#dialogt_gigi").dialog({
		  autoOpen: false,
		  width:335,
          modal:true,
