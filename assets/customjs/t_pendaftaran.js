jQuery().ready(function (){ 
	jQuery("#listt_pendaftaran").jqGrid({ 
		...
		colModel:[ 
				...
				{name:'aksi2',index:'aksi2', width:20,align:'center',formatter:formatterAction}
			],
			rowNum:5, 
			rowList:[5,10,25], 
	...
	jQuery("#listt_pendaftaranantrian").jqGrid({ 
		...
		emptyrecords: 'Tidak ada data',
		datatype: "xml",
		colNames:['KD_KUNJUNGAN','a','KD_PASIEN','KD_PUSKESMAS','Urut<br/>Masuk','Rekam<br/> Medis','Nama Pasien','Umur','KK','Alamat','Unit','Dokter','Status','Aksi'],
		rownumbers:true,
		width: 1049,
		height: 'auto',
		mtype: 'POST',
		...
		colModel:[ 
				...
				{name:'a',index:'a', width:5,hidden:true},
				{name:'kd_puskesmas',index:'kd_puskesmas', width:5,hidden:true},
			...
			subGridRowExpanded: function(subgrid_id, row_id) {
				...
				$("#"+subgrid_id).append(htm);
					...
					colModel: [ {name:"tgl_kunjungan",index:"tgl_kunjungan",width:80,sortable:false,align:'center'}, 
								{name:"poli",index:"poli",width:85,align:"center",sortable:false},
								{name:"diagnosa",index:"diagnosa",width:255,sortable:false}, 								
								//{name:"dosis",index:"dosis",width:215,align:"center",sortable:false}, 
								{name:"tindakan",index:"harga",width:225,align:"tindakan",sortable:false}, 
								{name:"obat",index:"obat",width:255,align:"center",sortable:false},
								],
					rowNum:5, 
					pager: pager_id_riwayat,
					beforeRequest:function(){
						$('#'+subgrid_table_id_riwayat).setGridParam({postData:{'kd_pasien':kd_pasien,'kd_puskesmas':kd_puskesmas,'kd_kunjungan':kd_kunjungan,'kd_pelayanan':kd_pelayanan}})
					}
				});
			},subGridBeforeExpand:function(subgrid_id,row_id){
				achtungShowLoader();
	...
	function formatterAction(cellvalue, options, rowObject) {
		...
		content  += '<a rel="' + cellvalue + '" class="icon-edit" title="Edit Pasien?"></a>';
		//content  += '<a rel="' + cellvalue + '" class="icon-delete" title="Remove?"></a>';
		return content;
	}
	
	$("#listt_pendaftaran .icon-edit").live('click', function(g){
		if($(g.target).data('oneclicked')!='yes')
		{
			$("#t202","#tabs").empty();
			$("#t202","#tabs").load('t_pendaftaran/edit'+'?kd_pasien='+this.rel);
		}
