jQuery().ready(function (){ 	
	jQuery("#listt_pelayananantrian").jqGrid({ 
		...
		datatype: "xml",
		colNames:['KD_KUNJUNGAN','a','KD_PASIEN','KD_PUSKESMAS','Urut<br/>Masuk','Rekam<br/> Medis','Nama Pasien','Umur','KK','Alamat','Unit','Petugas','Status','Aksi','SK'],
		rownumbers:true,
		width: 1049,
		height: 'auto',
		mtype: 'POST',
		...
		colModel:[ 
				...
				// {name:'kd_kunjungan',index:'kd_kunjungan', width:105},
			...
			subGridRowExpanded: function(subgrid_id, row_id) {
				...
				$("#"+subgrid_id).append(htm);
					...
					colModel: [ {name:'kd_kunjungan',index:'kd_kunjungan', width:5,hidden:true},
								{name:'kd_pelayanan',index:'kd_pelayanan', width:5,hidden:true},
								{name:'kd_puskesmas',index:'kd_puskesmas', width:5,hidden:true},
								{name:'idb',index:'idb', width:5,hidden:true},	
					...
					beforeRequest:function(){
						$('#'+subgrid_table_id_riwayat).setGridParam({postData:{'kd_pelayanan':kd_pelayanan,'kd_pasien':kd_pasien,'kd_puskesmas':kd_puskesmas,'kd_kunjungan':kd_kunjungan}})
					}
				});
				$("#"+subgrid_table_id_riwayat+" .icon-detail").live('click', function(h){
					if($(h.target).data('oneclicked')!='yes')
					{
						...
						var colid = colid1[0].id;
							...
							$("#t203","#tabs").empty();
							$("#t203","#tabs").load('t_pelayanan/detailkunjunganpasien'+'?kd_kunjungan='+this.rel+'&kd_pelayanan='+myCellDataId+'&kd_puskesmas='+myCellDataPuskesmas);
					}
