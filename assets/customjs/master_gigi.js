jQuery().ready(function (){ 
	jQuery("#listmastergigi").jqGrid({ 
		...
		datatype: "xml", 
		colNames:['Nomenklatur', 'Gambar', 'Nama','Action'],
		rownumbers:true,
		width: 1021,
		height: 'auto',
		mtype: 'POST',
		altRows     : true,		
		colModel:[ 
				{name:'kd_gigi',index:'kd_gigi', width:7, align:"center"}, 
			...
			beforeRequest:function(){
				kd_gigi=$('#kodemastergigi').val()?$('#kodemastergigi').val():'';
				nama_gigi = $("#namamastergigi").val()?$("#namamastergigi").val():'';
				$('#listmastergigi').setGridParam({postData:{'kd_gigi':kd_gigi, 'nama':nama_gigi}})
			}
	...
	function formatterImageGigi(cellValue, options, rowObject){
		var content = '';
		content += '<img src="./assets/images/gigi_master/'+cellValue+'" width="35" height="60"/>';
		return content;
	}
	...
	$("#listmastergigi .icon-detail").live('click', function(p){
		...
		{
		$("#t1001","#tabs").empty();
		$("#t1001","#tabs").load('c_master_gigi/detail'+'?kd_gigi='+this.rel);
		}	
	...
	$("#listmastergigi .icon-edit").live('click', function(p){
		...
		{
		$("#t1001","#tabs").empty();
		$("#t1001","#tabs").load('c_master_gigi/edit'+'?kd_gigi='+this.rel);
		}
		$(p.target).data('oneclicked','yes');
	});
