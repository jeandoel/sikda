...
class writeXmlElement
{
	...
	public static function writeXml4($title = 'Default', $results = array(), $xmlFile='about.xml')
	{
	...
	public static function writeXml5($title = 'Default', $results = array(), $xmlFile, $results_count = 0)
	{
		$str = $title;//strtolower($this->uri->segment(3));
		$xmlheader = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<$str></$str>";
		$xmlobj = simplexml_load_string($xmlheader);
...
class XmlToArray
{
	...
	public static function arraygetvalue($array){
		$lastArrayxml = array();
		foreach($array[0] as $row){
			if(isset($row['name']) && $row['name'] == 'DATA'){
				$array = array();
