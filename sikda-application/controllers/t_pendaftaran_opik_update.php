...
class T_pendaftaran extends CI_Controller {
	...
	public function addprocess()
	{
		...
		{						
			...
			$dataexc = array(
				...
				//'ANAK_KE' => $this->input->post('anak_ke')?$this->input->post('anak_ke',TRUE):NULL,
				//'BERAT_LAHIR' => $this->input->post('berat_lahir')?$this->input->post('berat_lahir',TRUE):NULL,
				//'LINGKAR_KEPALA' => $this->input->post('lingkar_kapala')?$this->input->post('lingkar_kepala',TRUE):NULL,
				//'PANJANG_BADAN' => $this->input->post('panjang_badan')?$this->input->post('panjang_badan',TRUE):NULL,
			...
			if($this->input->post('showhide_kunjungan')){
				...
				if($this->input->post('showhide_kunjungan')=='rawat_jalan'){
					...
					if($this->input->post('poliklinik')==219){
						if($this->input->post('kd_pasien_hidden')){
							$a = $db->query("SELECT f.KD_FAMILY_FOLDER FROM family_folder f JOIN pasien p ON f.ID_FAMILY_FOLDER=p.ID_FAMILY_FOLDER WHERE p.KD_PASIEN='".$this->input->post('kd_pasien_hidden')."'")->row();
						}else{
							$a = $db->query("SELECT f.KD_FAMILY_FOLDER FROM family_folder f JOIN pasien p ON f.ID_FAMILY_FOLDER=p.ID_FAMILY_FOLDER WHERE p.KD_PASIEN='".$dataexc['KD_PASIEN']."'")->row();
						}
						if($this->input->post('kategori_kia')==1){
	...
	public function pendaftaranpelayanan()
	{
		...
		$db->from('pasien p');
		$db->join('family_folder f','f.ID_FAMILY_FOLDER=p.ID_FAMILY_FOLDER');
