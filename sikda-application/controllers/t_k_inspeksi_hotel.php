...
class T_k_inspeksi_hotel extends CI_Controller {
	public function index()
	{
		$this->load->view('t_k_inspeksi_hotel/v_inspeksi_hotel');
	}
	
	public function inspeksihotelpopup()
	{
		$data['id_caller'] = $this->input->get('id_caller')?$this->input->get('id_caller',TRUE):null;
		$this->load->view('t_k_inspeksi_hotel/v_inspeksi_hotel_popup',$data);
	}
	
	public function inspeksihotelxml()
	{
		$this->load->model('t_k_inspeksi_hotel_model');
		...
		$total = $this->t_k_inspeksi_hotel_model->totalinspeksihotel($paramstotal);
		...
		$result = $this->t_k_inspeksi_hotel_model->getinspeksihotel($params);		
	...
	public function add()
	{
		...
		$this->load->view('t_k_inspeksi_hotel/v_inspeksi_hotel_add');
	}
	...
	public function addprocess()
	{
		...
		if(!in_array($ext, $allowed)) {
		  die('Hanya tipe file.xls, .xlsx, atau CSV yang Diperbolehkan.');
			...
			$newFilename = 'inspeksihotel/'.date("Ymd_His").$_FILES['filedok']['name'];
			if(move_uploaded_file($_FILES['filedok']['tmp_name'], TMP_PATH . $newFilename)){	
				...
				$val->set_rules('nama_hotel', 'Nama Hotel', 'trim|required|xss_clean');
				$val->set_rules('jumlah_karyawan', 'Jumlah Karyawan', 'trim|required|xss_clean');
				$val->set_rules('alamat', 'Alamat', 'trim|required|xss_clean');
				$val->set_rules('telp', 'Nomor Telepon', 'trim|required|xss_clean');
				...
				{						
					...
					$dataexc = array(
						...
						'NAMA_HOTEL' => $val->set_value('nama_hotel'),
						'ALAMAT' => $val->set_value('alamat'),
						'JUMLAH_KARYAWAN' => $val->set_value('jumlah_karyawan'),
						'TELP' => $val->set_value('telp'),
					...
					$db->insert('t_k_inspeksihotel', $dataexc);
	...
	{		
		...
		{						
			...
			$db->where('ID',$id);
			$db->update('t_k_inspeksihotel', $dataexc);
	...
	public function edit()
	{
		...
		$val = $db->query("select t.*,p.PROVINSI,k.KABUPATEN,kc.KECAMATAN,kl.KELURAHAN,DATE_FORMAT(TANGGAL_INSPEKSI, '%d/%m/%Y') as TANGGAL 
						from t_k_inspeksihotel t left join mst_provinsi p on p.KD_PROVINSI=t.KD_PROPINSI
						...
						where ID = '".$id."'")->row();
		$data['data']=$val;		
		$this->load->view('t_k_inspeksi_hotel/v_inspeksi_hotel_edit',$data);
	}
	...
	public function delete()
	{
		...
		$db = $this->load->database('sikda', TRUE);
		if($db->query("delete from t_k_inspeksihotel where ID = '".$id."'")){
			die(json_encode('OK'));
		}else{
	...
	public function detail()
	{
		...
		$val = $db->query("select t.*,p.PROVINSI,k.KABUPATEN,kc.KECAMATAN,kl.KELURAHAN,DATE_FORMAT(TANGGAL_INSPEKSI, '%d/%m/%Y') as TANGGAL 
						...
						where ID = '".$id."'")->row();
		$data['data']=$val;
		$this->load->view('t_k_inspeksi_hotel/v_inspeksi_hotel_detail',$data);
	}
	
}

/* End of file c_inspeksi_hotel.php */
/* Location: ./application/controllers/c_inspeksi_hotel.php */