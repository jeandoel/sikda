...
class t_registrasi_kia extends CI_Controller {
	...
	public function add()
	{
		...
		if($this->input->get('id')!=='undefined'){
			...
			$kd_family_query = "select KD_FAMILY_FOLDER 
								from family_folder a 
								join pasien b on a.ID_FAMILY_FOLDER=b.ID_FAMILY_FOLDER 
								where KD_PASIEN='".$id."'";

			$d = $db->query($kd_family_query)->row();
	...
	public function kesehatananak()
	{
		...
		if($this->input->get('id')!=='undefined'){
			...
			//<!-- DATA SUAMI/ayah -->
			$db->select("f.ID_FAMILY_FOLDER, p.NAMA_LENGKAP AS NAMA_AYAH, p.TEMPAT_LAHIR as TEMPAT_LAHIR_AYAH, Get_Age(p.TGL_LAHIR) as UMUR_AYAH, DATE_FORMAT(p.TGL_LAHIR, '%d-%m-%Y') as TGL_LAHIR_AYAH, p.KD_AGAMA as AGAMA_AYAH, p.KD_GOL_DARAH as DARAH_AYAH, p.KD_PENDIDIKAN as PEND_AYAH, p.KD_PEKERJAAN as KERJA_AYAH", FALSE);
			$db->from('family_folder f');
			$db->join('pasien p','f.ID_FAMILY_FOLDER=p.ID_FAMILY_FOLDER');
			$db->where('KD_FAMILY_FOLDER',$d->KD_FAMILY_FOLDER);
			$db->where('KD_STATUS_KELUARGA',1);
			...
			//<!-- DATA ISTRI/ibu -->
			$db->select("p.KD_PASIEN as KD_PASIEN,f.ID_FAMILY_FOLDER, p.NAMA_LENGKAP AS NAMA_IBU, p.TEMPAT_LAHIR as TEMPAT_LAHIR_IBU, Get_Age(p.TGL_LAHIR) as UMUR_IBU, DATE_FORMAT(p.TGL_LAHIR, '%d-%m-%Y') as TGL_LAHIR_IBU, p.KD_AGAMA as AGAMA_IBU, p.KD_GOL_DARAH as DARAH_IBU, p.KD_PENDIDIKAN as PEND_IBU, p.KD_PEKERJAAN as KERJA_IBU", FALSE);
			$db->from('family_folder f');
			$db->join('pasien p','f.ID_FAMILY_FOLDER=p.ID_FAMILY_FOLDER');
			$db->where('KD_FAMILY_FOLDER',$d->KD_FAMILY_FOLDER);
			$db->where('KD_STATUS_KELUARGA',2);
			$valsuibu = $db->get()->row();
			if(!empty($valsuibu->KD_PASIEN)){
				$kd = $db->query("select KD_KUNJUNGAN_BERSALIN,KET_TAMBAHAN from kunjungan_bersalin where KD_PASIEN='".$valsuibu->KD_PASIEN."'")->row();
				$data['data_kd']= $kd;
			}
	...
	public function imunisasi()
	{
		...
		$db->select("p.NAMA_LENGKAP AS NAMA_PASIEN,p.KD_PASIEN,p.KD_PUSKESMAS,p.TGL_LAHIR,p.NO_PENGENAL AS NIK,p.KET_WIL,p.KD_GOL_DARAH,p.KD_CUSTOMER,p.CARA_BAYAR,k.CUSTOMER,mu.UNIT,kj.KD_KUNJUNGAN,kj.KD_UNIT,kj.URUT_MASUK,kj.KD_LAYANAN_B",FALSE);
		$db->from('pasien p');
		...
		$data['data'] = $db->get()->row();
		$this->load->view('t_registrasi_kia/v_t_imunisasi',$data);		
	}
