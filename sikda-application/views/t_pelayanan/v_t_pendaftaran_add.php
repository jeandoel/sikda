...
$('document').ready(function() {		
	...
	$('#showhide_pasienbaru_kunjungan').on('change', function() {
		if($(this).val()=='rawat_jalan'){
			$('#showhide_pasien_kunjungan_rawat_jalan').show();
			$('#showhide_pasien_kunjungan_ugd').hide();
			$('#showhide_pasien_kunjungan_rawat_inap').hide();
		}else if($(this).val()=='UGD'){
			$('#showhide_pasien_kunjungan_ugd').show();
			$('#showhide_pasien_kunjungan_rawat_inap').hide();
			$('#showhide_pasien_kunjungan_rawat_jalan').hide();
		}else if($(this).val()=='rawat_inap'){
			$('#showhide_pasien_kunjungan_rawat_inap').show();
			$('#showhide_pasien_kunjungan_ugd').hide();
			$('#showhide_pasien_kunjungan_rawat_jalan').hide();
		}else{
			$('#showhide_pasien_kunjungan_rawat_jalan').hide();
...
<form name="frApps" id="form1t_pendaftaranadd" method="post" action="<?=site_url('t_pendaftaran/addprocess')?>" enctype="multipart/form-data">
	...
	<fieldset>
		...
		<label>Ket. Wilayah</label>
			<select name="wilayah">
				<option value="dalam_wilayah">Dalam Wilayah</option>
				<option value="luar_wilayah">Luar Wilayah</option>
			</select>
		</span>
	</fieldset>
	<fieldset>
	...
	<fieldset>
		<span>
		<label>Alamat*</label>
		<textarea name="column3s" rows="3" cols="45" required></textarea>
		</span>
	</fieldset>
	<fieldset>
	...
	<div id="showhide_pasien_kunjungan_rawat_jalan" style="display:none">		
		<fieldset>
			<span>
				<label>Tanggal Kunjungan*</label>
				<input type="text" name="tanggal_kunjungan_rawat_jalan" class="mydate" required  />
			</span>
		</fieldset>
		<?=getComboUnitlayanan('','unit_layanan_rawat_jalan','unit_layanan_rawat_jalant_pendaftaranpelayanan','required','')?>	
		<?=getComboPoliklinik('','poliklinik_rawat_jalan','poliklinik_rawat_jalant_pendaftaranpelayanan','required','')?>
		<fieldset>
			<span>
				<label>Petugas</label>
				<input type="text" name="petugas_rawat_jalan" disabled value="<?=$this->session->userdata('kd_petugas')?>"
			</span>
		</fieldset>
	</div>
	
	<div id="showhide_pasien_kunjungan_ugd" style="display:none">		
		<fieldset>
			<span>
				<label>Tanggal Kunjungan*</label>
				<input type="text" name="tanggal_kunjungan_ugd" class="mydate" required  />
			</span>
		</fieldset>
		<?=getComboUnitlayanan('','unit_layanan_ugd','unit_layanan_ugdt_pendaftaranpelayanan','required','')?>
		<fieldset>
			<span>
				<label>Petugas</label>
				<input type="text" name="petugas_ugd" disabled value="<?=$this->session->userdata('kd_petugas')?>"
			</span>
		</fieldset>
	</div>
	
	<div id="showhide_pasien_kunjungan_rawat_inap" style="display:none">		
		<fieldset>
			<span>
				<label>Tanggal Kunjungan*</label>
				<input type="text" name="tanggal_kunjungan_rawat_inap" class="mydate" required  />
			</span>
		</fieldset>
		<?=getComboSpesialisasi('','spesialisasi','spesialisasit_pendaftaranpelayanan','required','')?>	
		<?=getComboRuangan('','ruangan','ruangant_pendaftaranpelayanan','required','')?>
		<fieldset>
			<span>
			<label>Kelas</label>
				<select name="ruangan_rawat_inap" id="ruangan_rawat_inapid">
		...
		<fieldset>
			<?=getComboKamar('','kamar','kamart_pendaftaranpelayanan','required','inline')?>
			<span>
			<label>No. Tempat Tidur</label>
				<select name="no_tidur_inap">
					<option value="">-silahkan pilih-</option>
					<option value="1">1</option>
					<option value="2">2</option>
					<option value="3">3</option>
		...
		<fieldset>
			<span>
				<label>Petugas</label>
				<input type="text" name="petugas_rawat_inap" disabled value="<?=$this->session->userdata('kd_petugas')?>"  />
			</span>
		</fieldset>
	</div>
	<br/>
