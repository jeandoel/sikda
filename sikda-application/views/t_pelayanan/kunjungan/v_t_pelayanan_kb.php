...
$(document).ready(function(){	
		...
		$('#tambahtindakankbid').click(function(){
			if($('#produktindakankbsearch_tmpval').val()){
				...
				jQuery("#listtindakankb").addRowData(tindakankbid+1, myfirstrow);
		...
		$('#alergiobatkbselectid').change(function(){
			var c = this.checked ?'checked':'unchecked';
			if(c=='checked'){
				$('#showhidealergikbtable').show();
				$('#obatkbsearchalergikb').focus();
			}else{
				$('#showhidealergikbtable').hide();
		...
		$('#hapustindakankbid').click(function(){
			jQuery("#listtindakankb").clearGridData();
			}) 		
...
$(':text,:radio,:checkbox,select,textarea').bind("keydown", function(e) {
   ...
   if (e.which == 13) 
   {
		...
		if(nextIndex < n && $(this).valid()){
			if($(this).attr('id')=='keterangantindakankbid'){
				if($('#produktindakankbsearch_tmpval').val()){
					$('#tambahtindakankbid').focus();return false;
				}else{
					$('#alergiobatkbselectid').focus();
					$('#alergiobatkbselectid').css('outline-color', 'yellow');
					$('#alergiobatkbselectid').css('outline-style', 'solid');
			...
			if($(this).attr('id')=='kuantitasobatkbid'){				
				if($('#obat_tmpval').val()){
					$('#tambahobatkbid').focus();return false;
				}else{
					$('#pemeriksa').focus();
				}
			}
			if($(this).attr('id')=='obatkbsearchalergikb'){				
				if($('#obat_tmpvalalergi').val()){
					$('#tambahobatalergikbid').focus();return false;
				}else{
					$('#obatkbsearch').focus();
				}
			}
			
			if($(this).attr('id')=='obatkbsearch'){
				if($('#obat_tmptextnostock').val()=='no'){$( "#obat_tmptextnostock" ).val('');$( "#obatkbsearch" ).focus();return false;}
			}
			
			if($(this).attr('id')=='alergiobatkbselectid'){
				$( "#obatkbsearch" ).focus();
				$('#alergiobatkbselectid').css('outline-color', 'white');
			}
...
function kumpularray(){
		...
		if($('#listtindakankb').getGridParam("records")>0){
			var rows= jQuery("#listtindakankb").jqGrid('getRowData');
			var paras=new Array();
			for(var i=0;i<rows.length;i++){
				var row = rows[i];
				paras.push(JSON.stringify(row));
	...
	$('#t_pelayanan_kb_hidden').click(function(){
		var get_kd_pasien = $('#get_kd_pasien').val();
			...
			$('#dialogcari_t_pelayanan_kb_popup_id').load('t_pelayanan_kb/tpelayanankbpopup?id_caller=form1t_pelayanan_kb_add',
...
<form name="frApps" id="form1t_pelayanan_kb_add" onsubmit="bt1.disabled = true; return true;" method="post" action="<?=site_url('t_pelayanan_kb/addprocess')?>" enctype="multipart/form-data">
	<fieldset>
		<span>
	...
	<fieldset>
		...
		<input type="text" name="tanggalpemeriksaan" id="tanggal_daftart_pelayananaddrj" class="mydate" value="<?=date('d/m/Y')?>" required  />
		</span>
	</fieldset>
	<fieldset>
		<span>
		<label>Nama Suami</label>
		<input type="text" name="keluhan" id="text2" disabled=disabled value="<?=isset($datasuami->NAMA_SUAMI)?$datasuami->NAMA_SUAMI:'-'?>" required />
		</span>
	</fieldset>
	<fieldset>		
	...
	<fieldset>
		<span>
		<label>Keluhan</label>
		<textarea type="text" name="keluhan" cols="26" rows="5" id="text2" value="" required />
		</span>
	</fieldset>
	<fieldset>
		<span>
	...
	<fieldset>
		<span>
			...
			<input type="checkbox" name="alergiobat" id="alergiobatkbselectid" value="ya">
		</span>
	</fieldset>
	<div id="showhidealergikbtable" style="display:none">
