...
$('#id_kategori_kia').on('change', function() {
		if($(this).val()=='1'){
			$('#id_kunjungan_kia_ibu').show();
			$('#id_kunjungan_kia_anak').hide();
		}else{
			$('#id_kunjungan_kia_ibu').hide();
			$('#id_kunjungan_kia_anak').show();
		}
});
...
if (parts[4] == "1") 
{
	$("#id_kunjungan_kia_ibu").css('display','');
	$("#id_kunjungan_kia_anak").css('display','none');
}
else
{
	$("#id_kunjungan_kia_ibu").css('display','none');
	$("#id_kunjungan_kia_anak").css('display','');
}
...
<form name="frApps" id="form1t_pendaftaranedit" method="post" action="<?=site_url('t_pendaftaran/ubahkunjunganprocess')?>" enctype="multipart/form-data">
	...
	<fieldset id="edit_kategori_kia">		
		<?=getComboKategoriKIA($data->KD_KATEGORI_KIA,'kategori_kia','id_kategori_kia','','inline')?>
		<br/>
		<?=getComboPeriksaIbu($data->KD_KUNJUNGAN_KIA,'kunjungan_kia_ibu','id_kunjungan_kia_ibu','','inline')?>
		<?=getComboPeriksaAnak($data->KD_KUNJUNGAN_KIA,'kunjungan_kia_anak','id_kunjungan_kia_anak','','inline')?>
	</fieldset>
	<fieldset>
		<span>
