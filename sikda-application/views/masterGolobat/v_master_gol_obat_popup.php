...
jQuery().ready(function (){ 
	jQuery("#listmastergolobatpopup").jqGrid({ 
		...
		datatype: "xml", 
		colNames:['Pilih','Kode Gol. Obat','Golongan Obat','Action'],
		rownumbers:true,
		width: 1021,
		height: 'auto',
		mtype: 'POST',
		altRows     : true,		
		colModel:[ 
				{name:'id',index:'id', width:8,align:'center',hidden:false,formatter:formatterAction}, 
				{name:'kodgolobat',index:'kodgolobat', width:50}, 
		...
		$("#listmastergolobatpopup").find('input[type=checkbox]').each(function() {
			$(this).change( function(){
				...
				if( $(this).is(':checked')) {
					...
					var myCellDataColumn1 = jQuery('#listmastergolobatpopup').jqGrid('getCell', colid, 'golobat');
					$('#<?=$id_caller?> #kd_gol_obat').val(myCellDataId);
					$('#<?=$id_caller?> #master_gol_obat_id').val(myCellDataColumn1);
					if($('#<?=$id_caller?> #kd_gol_obat').val()=='VAKSIN'){
						$('#<?=$id_caller?> #imunisasiplaceholder').load('c_master_obat/imunisasi?_=' + (new Date()).getTime());
						$('#<?=$id_caller?> #imunisasiplaceholder').show();
					}else{
						$('#<?=$id_caller?> #imunisasiplaceholder').empty();
					}
