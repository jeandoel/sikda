		...
		$('#pilihkategorikia').on('change', function() {
			var id = $('#textid222').val();
			if($(this).val()==1){
				$('#kunjungankia_holder').empty();
				$('#kunjungankia_holder').load('t_registrasi_kia/add'+'?id='+id);
			}else if($(this).val()==2){
				$('#kunjungankia_holder').empty();
				$('#kunjungankia_holder').load('t_registrasi_kia/kesehatananak'+'?id='+id);
			}else if($(this).val()==3){
				$('#kunjungankia_holder').empty();
				$(':submit').focus();
			}else{
		...
		<?=getComboUnitlayanandsb($this->session->userdata('level_aplikasi'),'unit_layanan','unit_layanant_pendaftaranpelayanan','required','')?>	
