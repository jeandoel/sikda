	...
	jQuery("#listpuskesmaspopup").jqGrid({ 
		...
		colModel:[ 
			...
			beforeRequest:function(){
				...
				$('#listpuskesmaspopup').setGridParam({postData:{'nama':nama,'idkec':idkec}})
			}
		...
		$("#listpuskesmaspopup").find('input[type=checkbox]').each(function() {
			$(this).change( function(){
				...
				if( $(this).is(':checked')) {
					...
					var myCellDataNamapuskesmas2 = jQuery('#listpuskesmaspopup').jqGrid('getCell', colid, 'alamat');
					$('#<?=$id_caller?> #nama_puskesmas_hidden').val(myCellDataId);
					$('#<?=$id_caller?> #nama_puskesmas').val(myCellDataNamapuskesmas);
					$('#<?=$id_caller?> #nama_puskesmas_alamat').val(myCellDataNamapuskesmas2);
