...
$('document').ready(function(){
	...
	$("#carikia_excel").click(function(event){
		event.preventDefault();
		var from = $('#darikia').val();
		var to = $('#sampaikia').val();
		var pid = $('#idpkia').val();
		var jeniskia = $('#jeniskia').val();
		window.open('<?=site_url().'report_bulanan/lapkia_temp_excel?from='?>'+decodeURIComponent(from)+'&to='+decodeURIComponent(to)+'&pid='+decodeURIComponent(pid)+'&jenis='+decodeURIComponent(jeniskia));
	});
})
</script>
<style>
...
<div class="formtitle">Laporan Bulanan (kia)</div>
	<form id="formmasterkia">
		...
		<fieldset style="margin:0 13px 0 13px ">
						...
						<span>						
						<input type="submit" class="cari" value="&nbsp;Lihat Laporan&nbsp;" id="carikia"/>
						<input type="submit" class="cari" value="&nbsp;Export Laporan&nbsp;" id="carikia_excel"/>
