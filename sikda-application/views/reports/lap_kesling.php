...
$('document').ready(function(){
	...
	$('#darikesling').datepicker({dateFormat: "mm"});
	$('#sampaikesling').datepicker({dateFormat: "yy"});
	...
	$("#carikesling_excel").click(function(event){
		event.preventDefault();
		var from = $('#darikesling').val();
		var to = $('#sampaikesling').val();
		var pid = $('#idpkesling').val();
		var jeniskesling = $('#jeniskesling').val();
		window.open('<?=site_url().'report_bulanan/lapkesling_temp_excel?from='?>'+decodeURIComponent(from)+'&to='+decodeURIComponent(to)+'&pid='+decodeURIComponent(pid)+'&jenis='+decodeURIComponent(jeniskesling));
	});
})
</script>
<style>
...
<div class="formtitle">Laporan Bulanan (kesling)</div>
	<form id="formmasterkesling">
		...
		<fieldset style="margin:0 13px 0 13px ">
						...
						<span>						
						<input type="submit" class="cari" value="&nbsp;Lihat Laporan&nbsp;" id="carikesling"/>
						<input type="submit" class="cari" value="&nbsp;Export Laporan&nbsp;" id="carikesling_excel"/>
