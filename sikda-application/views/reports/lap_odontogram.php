...
$('document').ready(function(){	
	...
	$("#excel_report_odontogram").click(function(event){
		...
		var pid = $('#idsessgroup').val();
		var jenis_report_odontogram = $('#jenis_report_odontogram').val();
		window.open('<?=base_url().'report_bulanan/odontogram_excel?from='?>'+decodeURIComponent(from)+'&to='+decodeURIComponent(to)+'&pid='+decodeURIComponent(pid)+'&jenis='+decodeURIComponent(jenis_report_odontogram));
	});
})
</script>
<style>
...
<div class="mycontent">
<div class="formtitle">Laporan Odontogram</div>
	<form id="formmasterhrj">
		...
		<fieldset style="margin:0 13px 0 13px ">
						...
						<input type="hidden" name="jenis_report_odontogram" id="jenis_report_odontogram" value="<?=$this->session->userdata('group_id')=='kabupaten'?'rpt_odontogram_kb':'rpt_odontogram'?>" />
						</span>
		</fieldset>
	</form>
</div>