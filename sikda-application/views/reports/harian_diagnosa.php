...
$('document').ready(function(){	
	...
	$("#excelhrj").click(function(event){
		...
		var pid = $('#idphrj').val();
		var jenislk = $('#jenislk').val();
		window.open('<?=base_url().'report_harian/harian_diagnosa_excel?from='?>'+decodeURIComponent(from)+'&to='+decodeURIComponent(to)+'&pid='+decodeURIComponent(pid)+'&jenis='+decodeURIComponent(jenislk)+'&rc='+decodeURIComponent(rc));
	});
})
</script>
<style>
...
<div class="mycontent">
<div class="formtitle">Laporan Peringkat Diagnosa Penyakit</div>
	<form id="formmasterhrj">
		...
		<fieldset style="margin:0 13px 0 13px ">
						...
						<span>
						<label class="mydate">Jumlah</label>
						<input type="text" name="rc" style="width:40px" class="" id="rc" value="" /> Row
						</span>
						<span>						
