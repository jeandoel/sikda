	...
	$('#form1t_imunisasi_add').ajaxForm({
		...
		$('#showhide_jenis_pasien_kunjungan').on('change', function() {
			if($(this).val()=='1'){
				...
				$('#jenispasienplaceholder').load('t_imunisasi/siswa_anak_bayi_balita?kdpasien='+$('#kd_pasien').val());
			}else if($(this).val()=='2'){
				$('#jenispasienplaceholder').empty();
				$('#jenispasienplaceholder').load('t_imunisasi/siswa_anak_bayi_balita?kdpasien='+$('#kd_pasien').val());
			}else if($(this).val()=='3'){
				...
				$('#jenispasienplaceholder').empty();
				$('#jenispasienplaceholder').load('t_imunisasi/wustidakhamil?kdpasien='+$('#kd_pasien').val());
			}else if($(this).val()=='6'){
				$('#jenispasienplaceholder').empty();
				$('#jenispasienplaceholder').load('t_imunisasi/wushamil?kdpasien='+$('#kd_pasien').val());
			}else if($(this).val()=='7'){
				$('#jenispasienplaceholder').empty();
...
<form name="frApps" id="form1t_imunisasi_add" method="post" action="<?=site_url('t_imunisasi/addprocess')?>" enctype="multipart/form-data">
	...
	<fieldset>
		...
		<label>Kecamatan</label>
			<input type="text" name="kecamatanlokasi" id="kecamatant_imunisasiadd" value="<?=$data1->kecamatan?>" />
		</span>
	</fieldset>
	<fieldset>
		<span>
	...
	<fieldset>
		...
		<input type="text" name="nama_pasien" id="nama_pasien" value="<?=$data1->NAMA_LENGKAP?>" />
		<input type="hidden" name="kd_pasien" id="kd_pasien" value="<?=$data1->KD_PASIEN?>" />
		<input type="hidden" name="kd_trans_imunisasi_hidden" id="kd_pasien" value="<?=isset($data2->KD_TRANS_IMUNISASI)?$data2->KD_TRANS_IMUNISASI:''?>" />
		</span>
	</fieldset>
	<fieldset>
		<span>
		<label>Tanggal Lahir* (dd/mm/yyyy)</label>
		<input type="text" name="tanggal_lahir" id="tanggal_lahirt_pasienadd" class="mydate" value="<?=$data1->TGL_LAHIR?>" />		
		</span>
		<span>
		-
	...
	<?=getComboJeniskelamin($data1->KD_JENIS_KELAMIN,'jenis_kelamin','jenis_kelamint_pasienadd','required','')?>
	<fieldset>
		<span>
		<label>Alamat</label>
		<textarea name="alamatpasien" rows="3" cols="23" value=""><?=$data1->ALAMAT?></textarea>
		</span>
	</fieldset>
	<?=getComboProvinsi($data1->KD_PROVINSI,'provinsipasien','provinsit_pendaftaranadd','required','')?>
	<fieldset>
	<span>
	<label>Kab/Kota</label>
		<select name="kabupaten_kotapasien" id="kabupaten_kotat_pendaftaranadd">
			<option value="<?=$data1->KD_KABKOTA?>">--</option>
		</select>
	</span>
	</fieldset>
	<fieldset>
		...
		<label>Kecamatan</label>
			<select name="kecamatanpasien" id="kecamatant_pendaftaranadd">
				<option value="<?=$data1->KD_KECAMATAN?>">--</option>
			</select>
		</span>
	</fieldset>
	<fieldset>
		...
		<label>Desa/Kelurahan</label>
			<select name="desa_kelurahanpasien" id="kelurahant_pendaftaranadd">
				<option value="<?=$data1->KD_KELURAHAN?>">--</option>
			</select>
		</span>
	</fieldset>
	<fieldset>
