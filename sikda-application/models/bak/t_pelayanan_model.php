...
class T_pelayanan_model extends CI_Model {
  ...
  public function totalT_pelayanan($params)
  {
		...
		if($params['unit']!=='all'){
			$this->db->where('NAMA_UNIT =', $params['unit']);
		}
		if($params['status']){
			$this->db->where('STATUS =', $params['status']);
		}
		if($params['jenis']){
			$this->db->where('PARENT =', $params['jenis']);
		}
		$this->db->where ('KD_PUSKESMAS =', $this->session->userdata('kd_puskesmas'));		
		$this->db->from('vw_lst_antrian');
   ...
   public function getsubgridpeltranskia($params)
	{
		$this->db->select("p.KD_KUNJUNGAN,n.KD_PELAYANAN,p.KD_PUSKESMAS,p.KD_PASIEN, DATE_FORMAT(n.TGL_PELAYANAN,'%d-%m-%Y'),o.UNIT as unit,p.KD_DOKTER as pemeriksa, p.KD_DOKTER as petugas, p.KD_KUNJUNGAN as id",false);
